...
void Thread::Init(const ElfImg* handle) {
    if (Android::version == Android::kL || Android::version == Android::kLMr1) {
        ...
        alloc_non_movable = reinterpret_cast<void* (*)(void*, Thread*)>(handle->GetSymbolAddress(
                ...
                PINE_STR("_ZN3art6mirror5Class21AllocNonMovableObjectEPNS_6ThreadE")));
    }

    current = reinterpret_cast<Thread* (*)()>(handle->GetSymbolAddress(
            PINE_STR("_ZN3art6Thread14CurrentFromGdbEv"))); // art::Thread::CurrentFromGdb()

    if (UNLIKELY(!current && Android::version < Android::kN)) {
        current = reinterpret_cast<Thread* (*)()>(handle->GetSymbolAddress(
                PINE_STR("_ZN3art6Thread7CurrentEv"))); // art::Thread::Current()
        if (UNLIKELY(!current)) {
            key_self = static_cast<pthread_key_t*>(handle->GetSymbolAddress(
                    PINE_STR("_ZN3art6Thread17pthread_key_self_E"))); // art::Thread::pthread_key_self_
        }
    }

    new_local_ref = reinterpret_cast<jobject (*)(JNIEnv*, void*)>(handle->GetSymbolAddress(
            PINE_STR("_ZN3art9JNIEnvExt11NewLocalRefEPNS_6mirror6ObjectE"))); // art::JNIEnvExt::NewLocalRef(art::mirror::Object *)
    ...
    if (UNLIKELY(!new_local_ref)) {
        ...
        if (Android::version < Android::kM) {
            ...
            add_global_weak_ref_symbol = PINE_STR("_ZN3art9JavaVMExt22AddWeakGlobalReferenceEPNS_6ThreadEPNS_6mirror6ObjectE");
        } else if (Android::version < Android::kO) {
            // art::JavaVMExt::AddWeakGlobalRef(art::Thread *, art::mirror::Object *)
            add_global_weak_ref_symbol = PINE_STR("_ZN3art9JavaVMExt16AddWeakGlobalRefEPNS_6ThreadEPNS_6mirror6ObjectE");
        } else {
            // art::JavaVMExt::AddWeakGlobalRef(art::Thread *, art::ObjPtr<art::mirror::Object>)
            add_global_weak_ref_symbol = PINE_STR("_ZN3art9JavaVMExt16AddWeakGlobalRefEPNS_6ThreadENS_6ObjPtrINS_6mirror6ObjectEEE");
        }
    ...
    decode_jobject = reinterpret_cast<void* (*)(Thread*, jobject)>(handle->GetSymbolAddress(
            PINE_STR("_ZNK3art6Thread13DecodeJObjectEP8_jobject"))); // art::Thread::DecodeJObject(_jobject *)
}
