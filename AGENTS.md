...
| `debug.blackbox.maps_path_sanitize` | `true` | Enables app-visible `/proc/self/maps` path sanitization. If set to `0`, `false`, `no`, or `off`, native maps readers can see raw host/sandbox paths. |
