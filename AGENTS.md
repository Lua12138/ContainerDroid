...
| `-PblackboxGuardSignatureSha256=<csv>` | Empty string | `Bcore:black-binder` | Compile-time allowlist for the native host guard. Values are comma-separated lowercase or uppercase SHA-256 hex digests. The guard computes SHA-256 over decoded APK `META-INF/*.RSA`, `META-INF/*.DSA`, or `META-INF/*.EC` entries; it also accepts the embedded X.509 certificate SHA-256 to avoid APK v1-signing fixed-point rebuild issues when the allowlist itself is compiled into the native library. When empty, signature enforcement is disabled but TracerPid and breakpoint checks still run. |
| `-PblackboxGuardTracerIntervalSeconds=<int>` | `3` | `Bcore:black-binder` | Poll interval, in seconds, for the native TracerPid watchdog thread. Invalid, zero, or negative values are normalized to `3` at build time. |
| `-PblackboxGuardDiagnosticEnabled=<bool>` | `false` | `Bcore:black-binder` | Builds native host-guard diagnostic logcat output and links `liblog`. Default `false` compiles those diagnostics out, including their log strings. Use only for guard development. |
